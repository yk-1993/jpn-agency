...
export function EventCard({ event }: EventCardProps) {
  ...
  const lowestPrice = event.ticket_types.reduce((min, ticket) => (ticket.price < min ? ticket.price : min), event.ticket_types[0]?.price || 0);
  ...
  return (
    <Card className="overflow-hidden transition-all duration-200 hover:shadow-md">
      ...
      <CardContent className="p-4">
        <h3 className="text-lg font-bold mb-1 line-clamp-2">{language === "zn" ? event.name : event.name_jpn || event.name}</h3>
